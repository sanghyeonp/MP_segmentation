...
def evaluation(model, device, trans_name, fl_path, gt_path, state_dict_path, metrics, transformation=None, mode=None):
	"""
	Evaluate one fl image
	"""
	if mode == 'mean':
		save2 = os.path.join(os.getcwd(), 'testset_evaluation_mean_{}_thres0.5'.format(trans_name))
		if not os.path.exists(save2):
			os.mkdir(save2)

	elif mode == 'majority':
		save2 = os.path.join(os.getcwd(), 'testset_evaluation_majority_{}_thres0.5'.format(trans_name))
		if not os.path.exists(save2):
			os.mkdir(save2)

	else:
		save2 = os.path.join(os.getcwd(), 'testset_evaluation_{}_thres0.5'.format(trans_name))
		if not os.path.exists(save2):
			os.mkdir(save2)
	...
	save_image((~(pred_mask.bool())).float(), os.path.join(save2, fl_path.split(sep='/')[-1].split(sep='_')[0] + '_pred_mask.png'))
...
def testset_evaluation(model, device, trans_name, testset_path, state_dict_path, metrics, transformation=None, mode=None):
	"""
	...
	for p, fl_img_path in enumerate(tqdm(fl_imgs_path)):
		k_fold, img_num, performance_scores, conf = evaluation(	model=model, device=device, trans_name=trans_name, fl_path=fl_img_path, 
...
def eval4allcv(model, device, trans_name, testset_path, state_dir_path, metrics, transformation=None, mode=None):
	# # Initialize csv files
	# if mode == 'mean':
	# 	write2_performance = os.path.join(os.getcwd(), 'evaluation_performances_mean_{}.csv'.format(trans_name))
	...
	# 	write2_conf = os.path.join(os.getcwd(), 'evaluation_conf_mean_{}.csv'.format(trans_name))
	...
	# elif mode == 'majority':
	# 	write2_performance = os.path.join(os.getcwd(), 'evaluation_performances_majority_{}.csv'.format(trans_name))
	# 	perf_csv = open(write2_performance, 'wt', newline="")
	# 	perf_csv_writer = csv.writer(perf_csv)
	# 	perf_csv_writer.writerow(['k_fold', 'assigned_number', 'accuracy', 'recall', 'precision', 'fscore', 'iou'])

	# 	write2_conf = os.path.join(os.getcwd(), 'evaluation_conf_majority_{}.csv'.format(trans_name))
	# 	conf_csv = open(write2_conf, 'wt', newline="")
	# 	conf_csv_writer = csv.writer(conf_csv)
	# 	conf_csv_writer.writerow(['k_fold', 'assigned_number', 'tp', 'fn', 'fp', 'tn'])
	# else:
	# 	write2_performance = os.path.join(os.getcwd(), 'evaluation_performances_{}.csv'.format(trans_name))
	# 	perf_csv = open(write2_performance, 'wt', newline="")
	# 	perf_csv_writer = csv.writer(perf_csv)
	# 	perf_csv_writer.writerow(['k_fold', 'assigned_number', 'accuracy', 'recall', 'precision', 'fscore', 'iou'])

	# 	write2_conf = os.path.join(os.getcwd(), 'evaluation_conf_{}.csv'.format(trans_name))
	# 	conf_csv = open(write2_conf, 'wt', newline="")
	# 	conf_csv_writer = csv.writer(conf_csv)
	# 	conf_csv_writer.writerow(['k_fold', 'assigned_number', 'tp', 'fn', 'fp', 'tn'])	
	...
	for state_dict_path in tqdm(state_dirs_path):
		running_performances, running_conf = testset_evaluation(model=model, device=device, trans_name=trans_name,
...
if __name__ == '__main__':
	...
	transformations = {	'no_trans':None, 
				'HSV':[A.HueSaturationValue(hue_shift_limit=[-10, -10], sat_shift_limit=[50, 50], val_shift_limit=[10, 50], always_apply=True)], 
				'C_HSV':[A.RandomContrast(limit=[0.2, 0.6], always_apply=True), A.HueSaturationValue(hue_shift_limit=[-10, -10], sat_shift_limit=[50, 50], val_shift_limit=[10, 50], always_apply=True)],
				'C':[A.RandomContrast(limit=[0.2, 0.6], always_apply=True)],
				'B_HSV':[A.RandomBrightness(limit=[-0.2, 0.2], always_apply=True), A.HueSaturationValue(hue_shift_limit=[-10, -10], sat_shift_limit=[50, 50], val_shift_limit=[10, 50], always_apply=True)],
				'B_C_HSV':[A.RandomBrightness(limit=[-0.2, 0.2], always_apply=True), A.RandomContrast(limit=[0.2, 0.6], always_apply=True), A.HueSaturationValue(hue_shift_limit=[-10, -10], sat_shift_limit=[50, 50], val_shift_limit=[10, 50], always_apply=True)],
				'B_C':[A.RandomBrightness(limit=[-0.2, 0.2], always_apply=True), A.RandomContrast(limit=[0.2, 0.6], always_apply=True)],
				'B':[A.RandomBrightness(limit=[-0.2, 0.2], always_apply=True)]
				}
	#=======================================================================================================================#
	
	state_dir_names = [s for s in os.listdir(os.getcwd()) if 'final_best_models' in s]
	for i, state_dir_name in enumerate(state_dir_names):
		trans_name = state_dir_name[18:]
		print(trans_name)
		eval4allcv(model=model, trans_name=trans_name, device=device, testset_path=testset_path, 
			state_dir_path=os.path.join(os.getcwd(), state_dir_name), metrics=metrics, transformation=None, mode=None)
		if trans_name != 'no_trans':
			eval4allcv(model=model, device=device, trans_name=trans_name, testset_path=testset_path, 
				state_dir_path=os.path.join(os.getcwd(), state_dir_name), metrics=metrics, transformation=transformations[trans_name], mode='mean')
			eval4allcv(model=model, device=device, trans_name=trans_name, testset_path=testset_path, 
				state_dir_path=os.path.join(os.getcwd(), state_dir_name), metrics=metrics, transformation=transformations[trans_name], mode='majority')
