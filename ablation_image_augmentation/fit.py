...
def train(model, device, trans_name, cv_number, total_epoch, current_epoch, train_loader, criterion, optimizer, threshold=0.5):

	if os.path.exists(os.path.join(os.getcwd(), "train_loss_per_iter_{}.csv".format(trans_name))):
		open_mode = 'a'
	else:
		open_mode = 'wt'
	with open(os.path.join(os.getcwd(), "train_loss_per_iter_{}.csv".format(trans_name)), open_mode, newline='') as f:
...
def train_model(model, device, trans_name, total_epoch, train_loaders, val_loader, cv_number, criterion, optimizer, metrics, activation, threshold, writer):
	...
	for epoch in tqdm(range(total_epoch), desc="Epoch", leave=True):
		...
		if epoch + 1 <= 6:
			if not os.path.exists(os.path.join(currentPath, 'best_models_{}'.format(trans_name))):
				os.mkdir(os.path.join(currentPath, 'best_models_{}'.format(trans_name)))
			savePath = os.path.join(currentPath, 'best_models_{}'.format(trans_name))
		elif epoch + 1 <= 12:
			if not os.path.exists(os.path.join(currentPath, 'best_models_after_6_epochs_{}'.format(trans_name))):
				os.mkdir(os.path.join(currentPath, 'best_models_after_6_epochs_{}'.format(trans_name)))
			savePath = os.path.join(currentPath, 'best_models_after_6_epochs_{}'.format(trans_name))
		elif epoch + 1 <= 20:
			if not os.path.exists(os.path.join(currentPath, 'best_models_after_12_epochs_{}'.format(trans_name))):
				os.mkdir(os.path.join(currentPath, 'best_models_after_12_epochs_{}'.format(trans_name)))
			savePath = os.path.join(currentPath, 'best_models_after_12_epochs_{}'.format(trans_name))
		...
		for dataset_idx, train_loader in enumerate(tqdm(train_loaders, desc="Training datasets", leave=True)):
			train_loss, n_imgs = train(	model=model, device=device, trans_name=trans_name, cv_number=cv_number, total_epoch=total_epoch, current_epoch=epoch + 1, train_loader=train_loader, 
