...
transformations = [	None,
					A.Compose([A.RandomBrightness(limit=[-0.2, 0.2], always_apply=False)], p=0.5), 
					A.Compose([A.RandomContrast(limit=[0.2, 0.6], always_apply=False)], p=0.5), 
					A.Compose([A.HueSaturationValue(hue_shift_limit=[-10, -10], sat_shift_limit=[50, 50], val_shift_limit=[10, 50], always_apply=False)], p=0.5),
					A.Compose([A.RandomBrightness(limit=[-0.2, 0.2], always_apply=False), A.HueSaturationValue(hue_shift_limit=[-10, -10], sat_shift_limit=[50, 50], val_shift_limit=[10, 50], always_apply=False)], p=0.5),
					A.Compose([A.RandomContrast(limit=[0.2, 0.6], always_apply=False), A.HueSaturationValue(hue_shift_limit=[-10, -10], sat_shift_limit=[50, 50], val_shift_limit=[10, 50], always_apply=False)], p=0.5),
					A.Compose([A.RandomBrightness(limit=[-0.2, 0.2], always_apply=False), A.RandomContrast(limit=[0.2, 0.6], always_apply=False)], p=0.5),
					A.Compose([A.RandomBrightness(limit=[-0.2, 0.2], always_apply=False), A.RandomContrast(limit=[0.2, 0.6], always_apply=False), A.HueSaturationValue(hue_shift_limit=[-10, -10], sat_shift_limit=[50, 50], val_shift_limit=[10, 50], always_apply=False)], p=0.5)
					]
# transformation = None
#=======================================================================================================================#
...
trans_names = ['no_trans', 'B', 'C', 'HSV', 'B_HSV', 'C_HSV', 'B_C', 'B_C_HSV']

for t, transformation in enumerate(transformations):
	...
	training_csv = open(os.path.join(os.getcwd(), "training_process_{}.csv".format(trans_names[t])), 'wt', newline="")
	...
	for dataset_idx, dataset in enumerate(tqdm(datasets, desc='Cross validation')):
		...
		best_epoch, evaluations = train_model(	model=model_unet, trans_name=trans_names[t], device=device, total_epoch=total_epoch, 
		...
		break	# Test for 1 cross-validation
	...
	for testset_idx, result in enumerate(crossValidation_result):
		...
		print(result[1])
	print("======================================Training Completed For {}=================================".format(trans_names[t]))
