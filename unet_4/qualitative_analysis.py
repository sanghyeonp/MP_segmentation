...
for pred_dir in tqdm(pred_dirs):
	...
	cv_paths = sorted([os.path.join(os.getcwd(), pred_dir, cv) for cv in os.listdir(os.path.join(os.getcwd(), pred_dir))])
	for cv_path in cv_paths:
		cv_name = cv_path.split(sep='/')[-1]
		save222 = os.path.join(save22, cv_name)
		if not os.path.exists(save222):
			os.mkdir(save222)

		pred_img_paths = sorted([os.path.join(os.getcwd(), pred_dir, cv_name, p) for p in os.listdir(os.path.join(os.getcwd(), pred_dir, cv_name))])
		for i, gt_img_path in enumerate(tqdm(gt_img_paths)):
			pred_img_path = pred_img_paths[i]
			generateQualitativeImg(gt_path=gt_img_path, pred_path=pred_img_path, save2=save222)
