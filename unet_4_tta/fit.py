...
def train(model, device, cv_number, total_epoch, current_epoch, train_loader, criterion, optimizer, threshold=0.5):
	...
	if os.path.exists(os.path.join(os.getcwd(), "train_loss_per_iter_cv3_cv4.csv")):
		open_mode = 'a'
	else:
		open_mode = 'wt'
	with open(os.path.join(os.getcwd(), "train_loss_per_iter_cv3_cv4.csv"), open_mode, newline='') as f:
...
def train_model(model, device, total_epoch, train_loaders, val_loader, cv_number, criterion, optimizer, metrics, activation, threshold, writer):
	...
	for epoch in tqdm(range(total_epoch), desc="Epoch", leave=True):
		...
		if epoch + 1 <= 6:
			...
			if not os.path.exists(os.path.join(currentPath, 'best_models_after_6_epochs')):
				os.mkdir(os.path.join(currentPath, 'best_models_after_6_epochs'))
			savePath = os.path.join(currentPath, 'best_models_after_6_epochs')
		elif epoch + 1 <= 20:
			if not os.path.exists(os.path.join(currentPath, 'best_models_after_12_epochs')):
				os.mkdir(os.path.join(currentPath, 'best_models_after_12_epochs'))
			savePath = os.path.join(currentPath, 'best_models_after_12_epochs')
